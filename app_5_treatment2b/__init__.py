...
class VoteWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if won_vote and random.random() < C.CHAOS_PROB:
            is_removed = True; rep.removal_mechanism = 'unlucky_winner'
        elif not won_vote and random.random() > C.CHAOS_PROB:
            is_removed = True; rep.removal_mechanism = 'voted_out_probabilistic'
...
class Stage2Decision(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        # In T2b, show this to ANY removed representative.
