...
class VoteWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        elif not won_vote and random.random() > C.CHAOS_PROB:
            is_removed = True; mechanism = 'voted_out_probabilistic'
        
        if is_removed:
            rep.was_removed = True
            rep.removal_mechanism = mechanism
            session.vars['removed_reps_info'].append({'participant_id': rep.participant.id, 'mechanism': mechanism})
            if session.vars['rep_pool_pids']:
                session.vars['current_rep_pid'] = session.vars['rep_pool_pids'].pop(0)
                session.vars['term_round'] = 1
            else:
...
class Stage2Decision(Page):
    ...
    def is_displayed(player: Player):
        ...
        return my_removal_info is not None # In T2b, any removed rep makes a decision
    @staticmethod
